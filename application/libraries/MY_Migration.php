...
class MY_Migration
{
	...
	public function __construct($config = array())
	{
		...
		if (!$this->db->table_exists('otm_migrations')) {
			...
			$this->dbforge->create_table('otm_migrations', TRUE);

			$this->db->insert('otm_migrations', array('module' => 'Otm', 'version' => 0));
		}
	}
	...
	public function _check_module($module_name = '')
	{
		$check = false;
		if($module_name === ''){
			$module_name = $this->router->class;
		}

		if($module_name === 'Plugin_view'){
			$module_name = $this->router->method;
		}
		...
		$query = $this->db->get('otm_migrations');

		if($query->num_rows() >= 1)
		{
			foreach ($query->result() as $temp_row)
			{
				if($temp_row->version > 0){
					//show_error($temp_row->version);
					$check = true;
				}
			}
	...
	{		
		 ...
		 */
		$check = $this->_check_module();
		if($check)
		{
			//show_error('Module installed. <br><br><a href="/">HOME</a>');
		}
		else
		{
			if ($this->init_module($module_name)){
				if($this->version(1)){
					...
					show_error($module_name.' module install failed. <br><br>Please try installing again.');
				}
			}
			else
			{
				show_error('Module install error. <br><br>Please try installing again. <br><a href="./index.php/'.$module_name.'/install">INSTALL</a>');
			}
		}

		if($module_name === 'Otm'){
			redirect('./Plugin_view/defect/install', 'refresh');
		}else if($module_name === 'Plugin_view'){
			if($method === 'defect'){
				redirect('./Plugin_view/testcase/install', 'refresh');
		...
		while(false !== ($entry = $dirs->read())){
			if(($entry != '.') && ($entry != '..')) {
				if(is_dir($path.'/'.$entry) && is_dir($path.'/'.$entry.'/migrations')) { //is folder
					$entrys['dir'][] = $entry;
				}
			}
		}
	...
	public function check_version($version = NULL)
	{
		if($version == NULL)
		{
			show_error('Please select update Version.');
		}
		...
		if ($this->migration->init_module($module_name)){
			...
			{
				show_error('Stop update : update version('.$version.') <= install version('.$check['version'].') <br><br><a href="/">HOME</a>');
			}
			else
			{
		...
		{
			show_error('update error. <br><br><a href="/">HOME</a>');
		}
	}
	...
	public function list_all_modules_with_migrations()
	{
		...
		return array_merge(array(array('', 'Otm')), $modules);
	}
	...
	public function init_module($module = 'Otm')
	{
		if ($module === 'Otm') {
		...
		}else if($module === 'Plugin_view'){
			$module = $this->router->method;
			$config = $this->_core_config;
			//$config['migration_path'] == '' AND $config['migration_path'] = FCPATH.'/plugins/'.$this->router->method.'/migrations/';
			$config['migration_path'] = FCPATH.'/plugins/'.$this->router->method.'/migrations/';
			...
			*/
			...
			$config['migration_path'] = FCPATH.'/plugins/'.$module.'/migrations/';
	...
	public function version($target_version)
	{
		...
		for ($i = $start; $i != $stop; $i += $step) {
			...
			$f = glob(sprintf($this->_migration_path . '%03d_%s.php', $i,$module_name));
		...
		foreach ($migrations AS $migration) {
			...
			$this->_update_version($current_version, $module_name);
		}
...
// --------------------------------------------------------------------
	...
	protected function _get_version($module = '')
	{
		...
		$row = $this->db->get_where('otm_migrations', array('module' => $module))->row();
...
// --------------------------------------------------------------------
	...
	protected function _update_version($migrations, $module = '')
	{
		...
		$row = $this->db->get_where('otm_migrations', array('module' => $module))->row();
		if (count($row)) {
			return $this->db->where(array('module' => $module))->update('otm_migrations', array('version' => $migrations));
		} else {
			return $this->db->insert('otm_migrations', array('module' => $module, 'version' => $migrations));
		}
	}
...
// --------------------------------------------------------------------
	...
	function check_table($table_name,$str_sql)
	{
		...
		if( ! $result){
			...
			$query = $this->db->query('DROP TABLE '.$table_name);
			$query = $this->db->query($str_sql);
		}
	}
}
