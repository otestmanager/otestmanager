...
function code_get_store(url,params){
	...
	var store = Ext.create('Ext.data.Store', {
		fields:['pco_seq','pco_type','pco_name','pco_is_required','pco_is_default','pco_position','pco_default_value','pco_color'],
		proxy: {
			type	: 'ajax',
			url		: url,
			extraParams: params,
	...
	store.on('load',function(data){
		...
		if(Ext.getCmp('code_'+type+'_grid'))
		{
			if(data.lastOptions.selItem){
				var select = Ext.getCmp('code_'+type+'_grid').getStore().find('pco_seq', data.lastOptions.selItem.pco_seq);
				if(select > 0){
	...
	switch(type){
		case "tc_id_rule":
			columns = [
				...
				{header: Otm.id_rule.tc_id_rule,dataIndex: 'pco_name',		flex: 3},
				{header: Otm.com_default_value,	dataIndex: 'pco_is_default', width:100, align:'center', renderer: renderer_YorN}
			];

			url = './index.php/Project_setup/id_rule_list/tc_id_rule';
			break;
		case "df_id_rule":
			columns = [
				{xtype: 'rownumberer',width: 30,sortable: false},
				{header: Otm.id_rule.df_id_rule,dataIndex: 'pco_name',		flex: 3},
				{header: Otm.com_default_value,	dataIndex: 'pco_is_default', width:100, align:'center', renderer: renderer_YorN}
			];

			url = './index.php/Project_setup/id_rule_list/df_id_rule';
			break;
	}
		...
		listeners: {
			...
			select: function(item, record, eOpts ){
				...
				if(record){
					...
					Ext.getCmp(type+'_sample').setValue(record.data.pco_name);
					...
					if(!record.data.pco_default_value || record.data.pco_default_value.length <= 0) return;

					var id_rule_path = record.data.pco_default_value;
			...
			{xtype: 'button', text: Otm.com_remove, iconCls:'ico-remove', handler: function() {
					...
					if(Records.length >= 1){
						for(var i=0; i<Records.length; i++){
							if(Records[i].data['pco_is_default'] == 'Y'){
								Ext.MessageBox.show({
					...
					Ext.Msg.confirm('OTM',Otm.com_msg_deleteConfirm,function(bt){
						if(bt=='yes'){
							...
							if(Records.length >= 1){
								...
								var url = './index.php/Project_setup/delete_id_rule';
...
function check_id_rule(type)
{
	...
	if(post_type=='update')
	{
		...
		if(selItem){
			var params = {
				...
				pco_type		: type,
				before_name		: selItem.data.pco_name,
				pco_name		: Ext.getCmp(type+'_sample').getValue(),
				pco_default_value: Ext.getCmp(type+'_default_value').getValue()
			};
			...
			var is_default = (selItem.data.pco_is_default == 'Y')?true:false;
			if(Ext.getCmp(type+'_is_default').getValue() == true && (Ext.getCmp(type+'_is_default').getValue() != is_default)){
				if(type == 'tc_id_rule'){
					before_save_msg = Otm.id_rule.tc_update_msg;
				}else if(type == 'df_id_rule'){
					before_save_msg = Otm.id_rule.df_update_msg;
				}
			}
		...
		if(Ext.getCmp(type+'_is_default').getValue() == true){
			if(type == 'tc_id_rule'){
				before_save_msg = Otm.id_rule.tc_add_msg;
			}else if(type == 'df_id_rule'){
				before_save_msg = Otm.id_rule.df_add_msg;
			}
		}
	...
	Ext.Ajax.request({
		url		: './index.php/Project_setup/check_id_rule',
		params	: params,
		method	: 'POST',
		success	: function ( result, request ) {
...
function code_create_code(params){
	var type = params.pco_type;

	Ext.Ajax.request({
		url		: './index.php/Project_setup/create_id_rule',
		params	: params,
		method	: 'POST',
		success	: function ( result, request ) {
			if(result.responseText=="ok"){
...
function code_update_code(params){
	...
	Ext.Ajax.request({
		url		: './index.php/Project_setup/update_id_rule',
		params	: params,
		method	: 'POST',
		success	: function ( result, request ) {
			if(result.responseText=="ok"){
...
function get_form(type){
	...
	switch(type){
		case "tc_id_rule":
			...
			break;
		case "df_id_rule":
			var combo_store = code_get_store('./index.php/Project_setup/id_rule_list/tc_id_rule',{xtype:'combo'});
			items = [{
				xtype		: 'hiddenfield',
				id			: type+'_save_type'
			},{
				...
				emptyText	: '{고정 값}{등록 날짜 형식}{구분자}{일련번호자리}',
				id			: type+'_sample',
				height		: 50,
				shrinkWrap	: 0,
				overflowY	: 'scroll',
...
Ext.onReady(function(){
	...
	Ext.getCmp('project_setup_id_rule').add(main_panel);
	Ext.getCmp('project_setup_id_rule').doLayout();
});
</script>