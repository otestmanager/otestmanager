...
var main_center = {
	...
	items	:[{
		...
		items: [
		{
			...
			store	: new Ext.data.SimpleStore({
				fields:['name', 'writer', 'member'],
				data:[['- 테스트 케이스',true,true],[' 추가',true,true],[' 실행 담당자(지정 / 취소)',true,true],[' 수정',true,true],[' 실행',true,true],[' 삭제',true,true]]
			}),
			viewConfig: {
                listeners: {
                    refresh: function(view) {
			...
			store	: new Ext.data.SimpleStore({
				fields:['name', 'writer', 'member'],
				data:[['- 결함',true,true],[' 추가',true,true],[' 결함 상태 변경',true,true],[' 상태 담당자 지정',true,true],[' 수정',true,true],[' 삭제',true,true]]
			}),
			viewConfig: {
                listeners: {
...
Ext.onReady(function(){
	...
	Ext.getCmp('project_setup_notification').add(main_panel);
	Ext.getCmp('project_setup_notification').doLayout();
});
</script>
